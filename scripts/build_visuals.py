...
def build_hero() -> Image.Image:
    ...
    rounded_label(draw, (86, 54), "PUBLISH YOUR PROJECT", "#263650", AMBER)
    draw.text((84, 126), "Project", font=font(76, True), fill=WHITE)
    draw.text((84, 208), "Publisher", font=font(70, True), fill=WHITE)
    draw.text((90, 315), "Make your project easy to understand.", font=font(31, True), fill="#D9E3F0")
    draw.text((90, 360), "Keep its public materials up to date.", font=font(28), fill="#D9E3F0")
    draw.line((90, 428, 640, 428), fill="#52637A", width=2)
    draw.text((90, 458), "POSITION", font=font(19, True), fill=AMBER)
    draw.text((218, 458), "README", font=font(19, True), fill=AMBER)
    draw.text((327, 458), "RELEASE", font=font(19, True), fill=AMBER)
    draw.text((448, 458), "SHARE", font=font(19, True), fill=AMBER)
    draw.text((594, 458), "UPDATE", font=font(19, True), fill=MINT)
    draw.text((90, 535), "The first review changes nothing. You approve every edit and GitHub action.", font=font(22), fill=SLATE)
...
def build_hero_zh() -> Image.Image:
    ...
    image = darken_left(image)
    draw = ImageDraw.Draw(image)
    draw.rounded_rectangle((70, 43, 342, 94), radius=24, fill="#263650")
    draw.text((91, 55), "项目公开与传播", font=zh_font(22, True), fill=AMBER)
    draw.text((84, 126), "Project", font=font(70, True), fill=WHITE)
    draw.text((84, 204), "Publisher", font=font(66, True), fill=WHITE)
    draw.text((90, 315), "让别人看懂项目为什么值得试", font=zh_font(32, True), fill="#D9E3F0")
    draw.text((90, 360), "项目更新以后，公开材料也跟着更新", font=zh_font(28), fill="#D9E3F0")
    draw.line((90, 428, 680, 428), fill="#52637A", width=2)
    labels = [(90, "定位", AMBER), (190, "README", AMBER), (330, "Release", AMBER), (470, "宣发", AMBER), (575, "后续更新", MINT)]
    for x, label, color in labels:
        draw.text((x, 458), label, font=zh_font(22, True), fill=color)
    draw.text((90, 535), "第一轮只读。改文件和公开操作都由你确认。", font=zh_font(22), fill=SLATE)
    return image.convert("RGB")
